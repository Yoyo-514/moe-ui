...
describe('Dropdown.vue', () => {
  ...
  describe('split button', () => {
    ...
    it('passes offset to tooltip popper modifier', async () => {
      ...
      const popperCalls = popper.createPopper.mock.calls as unknown as Array<
        [unknown, unknown, { modifiers?: unknown[] }]
      >
      const popperOptions = popperCalls[0]?.[2]

      expect(popperOptions?.modifiers).toContainEqual({
        name: 'offset',
        options: {
          offset: [0, 24],
        },
    ...
    it('uses caret button as popper reference in split button mode', async () => {
      ...
      const reference = popper.createPopper.mock.calls[0]?.[0] as HTMLElement
